    ...
    while True:
        ...
        for i, attribute_name in enumerate(attribute_names):
            if i == current_attribute:
                ...
                print(f"-> {attribute_name}: {results[current_record][i]}\033[K", end='')
            else:
        ...
        if key == b'\x1b':
            ...
            # Enter edit mode for the current attribute
            print("\n\n")  # Move cursor down to avoid overwriting previous line
            new_value = input(f"{attribute_names[current_attribute]}: ").strip()
            if new_value:
                # Update the database
...
# Example usage: geschiedenis('database.db')
